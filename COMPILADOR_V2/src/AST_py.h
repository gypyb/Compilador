...
// Estructura de tipo de data que almacenna el nodo del arbol
union astValor {
    double valorDouble;
    char *valorString;
    int *array;
...
struct ast {
    ...
    char *tipo;          // El tipo de data que almacena: numericoDecimal, numerico o string
...
struct valorNodoRetorno comprobarValorNodo(struct ast *n, int contadorEtiquetaLocal) {
    struct valorNodoRetorno data;

    //TIPO NODO 1 - Nueva hoja en el arbol
    if (n->tipoNodo == 1) {
        if (strcmp(n->tipo, "texto") == 0) {
            data.texto = n->valorNodo.valorString;
            fprintf(yyout, "la $t%d, var_%d\n", n->resultado, n->nombreVar);
        } else if (strcmp(n->tipo, "numerico") == 0 || strcmp(n->tipo, "numericoDecimal") == 0) {
            data.valor = n->valorNodo.valorDouble;
            fprintf(yyout, "lwc1 $f%d, var_%d\n", n->resultado, n->nombreVar);
        } else if (strcmp(n->tipo, "array") == 0) {
            data.array = n->valorNodo.array;
            fprintf(yyout, "lwc1 $f%d, var_%d\n", n->resultado, n->nombreVar);
        }
    //TIPO NODO 2 - Nueva suma
        ...
        if (strcmp(n->tipo, "texto") == 0) {
            ...
            data.texto = malloc(strlen(izqString) + strlen(dchaString) + 1);
            strcpy(data.texto, izqString);
            strcat(data.texto, dchaString);
          ...
          struct valorNodoRetorno izq = comprobarValorNodo(n->izq, contadorEtiquetaLocal);
          struct valorNodoRetorno dcha = comprobarValorNodo(n->dcha, contadorEtiquetaLocal);

          if (strcmp(n->izq->tipo, "numerico") == 0 && strcmp(n->dcha->tipo, "numerico") == 0) {
              data.valor = izq.valor + dcha.valor;
              fprintf(yyout, "add.s $f%d, $f%d, $f%d\n", n->resultado, n->izq->resultado, n->dcha->resultado);
          } else if (strcmp(n->izq->tipo, "numericoDecimal") == 0 && strcmp(n->dcha->tipo, "numericoDecimal") == 0) {
              data.valor = izq.valor + dcha.valor;
              fprintf(yyout, "add.s $f%d, $f%d, $f%d\n", n->resultado, n->izq->resultado, n->dcha->resultado);
          } else if (strcmp(n->izq->tipo, "numerico") == 0 && strcmp(n->dcha->tipo, "numericoDecimal") == 0) {
              data.valor = izq.valor + dcha.valor;
              fprintf(yyout, "cvt.s.w $f%d, $f%d\n", n->izq->resultado, n->izq->resultado); // Convierte numerico a numericoDecimal
              fprintf(yyout, "add.s $f%d, $f%d, $f%d\n", n->resultado, n->izq->resultado, n->dcha->resultado);
          } else if (strcmp(n->izq->tipo, "numericoDecimal") == 0 && strcmp(n->dcha->tipo, "numerico") == 0) {
              data.valor = izq.valor + dcha.valor;
              fprintf(yyout, "cvt.s.w $f%d, $f%d\n", n->dcha->resultado, n->dcha->resultado); // Convierte numerico a numericoDecimal
              fprintf(yyout, "add.s $f%d, $f%d, $f%d\n", n->resultado, n->izq->resultado, n->dcha->resultado);
          }
    ...
    //TIPO NODO 5 - Nueva asignación
        ...
        data = comprobarValorNodo(n->izq, contadorEtiquetaLocal);
    ...
    //TIPO NODO 8 - Nueva división
    } else if (n->tipoNodo == 8) {
        data.valor = comprobarValorNodo(n->izq, contadorEtiquetaLocal).valor / comprobarValorNodo(n->dcha, contadorEtiquetaLocal).valor;
        fprintf(yyout, "div.s $f%d, $f%d, $f%d\n", n->resultado, n->izq->resultado, n->dcha->resultado); //se utiliza div.s para - en ASM
        borrarReg(n->izq, n->dcha); //borrado de registros (se ponen a true)

    //TIPO NODO 9 - Nueva multiplicación
    ...
    //TIPO NODO 11 - Nueva MAYOR_IGUAL_QUE
    ...
    //TIPO NODO 13 - Nueva MENOR_IGUAL_QUE
    ...
    //TIPO NODO 14 - Nueva IGUAL_IGUAL
    ...
    //TIPO NODO 21 - Nuevo bucle while
    ...
    //TIPO NODO 22 - Nuevo bucle for
    } else if (n->tipoNodo == 22) {
        int etiqueta = contadorEtiquetaLocal;
        contadorEtiquetaLocal++;
        fprintf(yyout,"l.s $f29, zero\n"); //cargar esto al final de zero nuevamente
        data.valor = comprobarValorNodo(n->izq, contadorEtiquetaLocal).valor;
        fprintf(yyout, "etiqueta%d:\n", etiqueta);
...
void funcionImprimir(struct ast *n) {
  if(strcmp(n -> tipo, "texto") == 0) {
    ...
    fprintf(yyout, "li $v0, 2\n"); // syscall para impresión de flotantes
    fprintf(yyout, "mov.s $f12, $f%d\n", n -> resultado); // Mover el valor a $f12 para imprimir
    fprintf(yyout, "syscall #Llamada al sistema\n");
  ...
  for (int i = 0; i < 64; i++) {
      if(variables[i].disponible == true) {
          if (variables[i].texto != NULL) {
              ...
              fprintf(yyout, "var_%d: .float %.3f\n", variables[i].nombre, variables[i].data);
          }
      }
  }
}
...
struct ast *crearNodoTerminal(double valor , char *tipo) {
    ...
    variables[n->resultado].data = n->valorNodo.valorDouble;
